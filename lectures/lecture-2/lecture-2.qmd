...
// Generate Redlich--Kwong predicted curve.
// The curve is split into segments so that Observable Plot does not
// connect across invalid or negative-pressure regions.
rkCurve = {
  const n = 10000
  ...
  for (let i = 0; i < n; i++) {
    const v = Math.pow(10, logMin + i*(logMax - logMin)/(n - 1))
    const P = P_RK(v)
    ...
    // Break the line if neighboring points have a large pressure jump.
    // This avoids artificial vertical lines on a log-pressure plot.
    if (previous !== null) {
      const jump = Math.abs(Math.log10(P) - Math.log10(previous.P))
...
Plot.plot({
  ...
  marks: [
    // Saturation dome: saturated liquid branch
    Plot.line(satLiquid, {
      x: "v",
      y: "P",
      stroke: "black",
    ...
    // Saturation dome: saturated vapor branch
    Plot.line(satVapor, {
      x: "v",
      y: "P",
      stroke: "black",
